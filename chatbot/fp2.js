...
(function (name, context, definition) {
  ...
  const loadSwfAndDetectFonts = function (done, options) {
  ...
  const hasMinFlashInstalled = function () {
    return window.swfobject.hasFlashPlayerVersion('9.0.0')
  };
  const hasSwfObjectLoaded = function () {
    return typeof window.swfobject !== 'undefined'
  };
  const isIEOrOldEdge = function () {
...
// and the availability of the 'ontouchstart' property
  const getTouchSupport = function () {
  ...
  const getDoNotTrack = function (options) {
  ...
  const getNavigatorPlatform = function (options) {
  ...
  const getNavigatorCpuClass = function (options) {
    return navigator.cpuClass || options.NOT_AVAILABLE
  };
  const getHardwareConcurrency = function (options) {
    if (navigator.hardwareConcurrency) {
      return navigator.hardwareConcurrency
    }
    return options.NOT_AVAILABLE
  };
  const hasIndexedDB = function (options) {
...
// https://bugzilla.mozilla.org/show_bug.cgi?id=781447
  const hasLocalStorage = function (options) {
    try {
  ...
  const hasSessionStorage = function (options) {
    try {
  ...
  const pluginsShouldBeSorted = function (options) {
    var should = false
  ...
  const getIEPlugins = function (options) {
    var result = []
  ...
  const getRegularPlugins = function (options) {
  ...
  let hasLiedLanguagesKey = function (done) {
    done(getHasLiedLanguages())
  }
  let hasLiedResolutionKey = function (done) {
    done(getHasLiedResolution())
  }
  let hasLiedOsKey = function (done) {
    done(getHasLiedOs())
  }
  let hasLiedBrowserKey = function (done) {
    done(getHasLiedBrowser())
  }
  // flash fonts (will increase fingerprinting time 20X to ~ 130-150ms)
  let flashFontsKey = function (done, options) {
  ...
  let jsFontsKey = function (done, options) {
    ...
    let initializeBaseFontsSpans = function () {
      let spans = []
      let index = 0, length = baseFonts.length;
  ...
  Fingerprint2.getV18 = function (options, callback) {
    ...
    return Fingerprint2.get(options, function (components) {
        ...
        if (component.value === (options.NOT_AVAILABLE || 'not available')) {
          ...
          newComponents.push({
            ...
            value: map(component.value, function (p) {
              const mimeTypes = map(p[2], function (mt) {
                if (mt.join) {
                  return mt.join('~')
                }
                return mt
      ...
      const murmur = x64hash128(map(newComponents, function (component) {
